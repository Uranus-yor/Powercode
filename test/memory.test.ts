...
function makeTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'PowerCode-memory-test-'))
}

function makeHomeDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'PowerCode-memory-home-'))
}
...
async function discoverTestFiles(cwd: string, homeDir = makeHomeDir()) {
  try {
    ...
    if (path.basename(homeDir).startsWith('PowerCode-memory-home-')) {
...
describe('discoverInstructionFiles', () => {
  ...
  test('finds .powercode/MINI.md', async () => {
    const dir = makeTempDir()
    try {
      write(dir, '.powercode/MINI.md', 'mini-code instructions')
      const files = await discoverTestFiles(dir)
      assert.equal(files.length, 1)
  ...
  test('loads user global MINI.md from home directory', async () => {
    const dir = makeTempDir()
    const PowerCodeHome = makeTempDir()
    try {
      write(PowerCodeHome, 'MINI.md', 'global rules')
      const files = await discoverInstructionFiles(dir, PowerCodeHome, dir)
      const globalFile = files.find(f => f.content === 'global rules')
      assert.ok(globalFile)
    } finally {
      fs.rmSync(dir, { recursive: true, force: true })
      fs.rmSync(PowerCodeHome, { recursive: true, force: true })
    }
  })
  ...
  test('discovers .powercode/rules/*.md files sorted by filename', async () => {
    const dir = makeTempDir()
    try {
      write(dir, '.powercode/rules/testing.md', 'testing rules')
      write(dir, '.powercode/rules/typescript.md', 'typescript rules')
      write(dir, '.powercode/rules/ignore.txt', 'ignored')
      write(dir, '.powercode/rules/a-first.md', 'first rules')
...
describe('memory report', () => {
  test('describes files with scope and preview', () => {
    const dir = makeTempDir()
    try {
      const filePath = write(dir, '.powercode/rules/testing.md', '# Testing\nUse tests')
