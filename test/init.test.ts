...
function makeTempDir(): string {
  const dir = path.join(os.tmpdir(), `PowerCode-init-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`)
...
describe('initializeRepo', () => {
  test('creates expected artifacts', async () => {
    ...
    try {
      ...
      assert.equal(report.artifacts[0].name, '.powercode/')
      ...
      assert.ok(existsSync(path.join(dir, '.powercode')))
      assert.ok(existsSync(path.join(dir, 'MINI.md')))

      const gitignore = readFileSync(path.join(dir, '.gitignore'), 'utf8')
      assert.ok(gitignore.includes('.powercode/settings.local.json'))
      assert.ok(gitignore.includes('.powercode/sessions/'))
      assert.ok(gitignore.includes('# PowerCode local artifacts'))
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
  })
  ...
  test('appends to existing .gitignore without duplicating entries', async () => {
    const dir = makeTempDir()
    try {
      writeFileSync(path.join(dir, '.gitignore'), '.powercode/sessions/\n')
      await initializeRepo(dir)
      const gitignore = readFileSync(path.join(dir, '.gitignore'), 'utf8')
      assert.equal(gitignore.match(/\.powercode\/settings\.local\.json/g)?.length, 1)
      assert.equal(gitignore.match(/\.powercode\/sessions\//g)?.length, 1)
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
  })
  ...
  test('creates .gitignore when it does not exist', async () => {
    ...
    try {
      ...
      assert.equal(lines[0], '# PowerCode local artifacts')
      assert.ok(lines.includes('.powercode/settings.local.json'))
      assert.ok(lines.includes('.powercode/sessions/'))
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
  })
...
describe('renderInitReport', () => {
  test('formats artifacts with status labels', () => {
    const report = {
      ...
      artifacts: [
        { name: '.powercode/', status: 'created' as const },
    ...
    assert.ok(rendered.includes('.powercode/'))
