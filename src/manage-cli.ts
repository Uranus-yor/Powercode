...
function printUsage(): void {
  console.log(`powercode management commands

powercode mcp list [--project]
powercode mcp add <name> [--project] [--protocol <auto|content-length|newline-json|streamable-http>] [--url <endpoint>] [--header KEY=VALUE ...] [--env KEY=VALUE ...] [-- <command> [args...]]
powercode mcp login <name> --token <bearer-token>
powercode mcp logout <name>
powercode mcp remove <name> [--project]

powercode skills list
powercode skills add <path-to-skill-or-dir> [--name <name>] [--project]
powercode skills remove <name> [--project]`)
}
...
async function handleMcpCommand(cwd: string, args: string[]): Promise<boolean> {
  ...
  if (subcommand === 'login') {
    ...
    console.log(`Stored MCP token for ${name} in ${POWER_CODE_MCP_TOKENS_PATH}`)
    return true
  }
  ...
  if (subcommand === 'logout') {
    ...
    const tokens = await readMcpTokensFile()
    if (!(name in tokens)) {
      console.log(`No token found for ${name} in ${POWER_CODE_MCP_TOKENS_PATH}`)
      return true
    }
    delete tokens[name]
    await saveMcpTokensFile(tokens)
    console.log(`Removed MCP token for ${name} from ${POWER_CODE_MCP_TOKENS_PATH}`)
    return true
  }
