...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log('PowerCode installer')
    console.log(`配置会写入 ${POWER_CODE_SETTINGS_PATH}`)
    ...
    const targetBinDir = process.env.POWER_CODE_BIN_DIR
      ? path.resolve(process.env.POWER_CODE_BIN_DIR)
      : path.join(home, '.local', 'bin')
    const launcherPath = path.join(targetBinDir, 'powercode')
    const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..')
    const launcherScript = [
      '#!/usr/bin/env bash',
      'set -euo pipefail',
      `exec "${path.join(repoRoot, 'bin', 'powercode')}" "$@"`,
      '',
    ].join('\n')
    ...
    console.log('')
    console.log('安装完成。')
    console.log(`配置文件: ${POWER_CODE_SETTINGS_PATH}`)
    ...
    if (!hasPathEntry(targetBinDir)) {
      ...
      console.log('')
      console.log('现在你可以在任意终端输入 `PowerCode` 启动。')
    }
  } finally {
    rl.close()
  }
