...
function getTranscriptBodyLines(_args: TtyAppArgs, state: ScreenState): number {
  ...
  const inputPanelLines = 6  // top + input + empty + brand + bottom + gap
  const transcriptPanelFrameLines = 4
  const gapsBetweenSections = 1
  const remaining =
    rows -
    inputPanelLines -
...
function renderScreen(args: TtyAppArgs, state: ScreenState): void {
  ...
  if (state.pendingApproval) {
    parts.push(
      ...
      '',
      renderPanel('activity', renderToolPanel(state.activeTool, state.recentTools, backgroundTasks)),
      '',
      renderFooterBar(
        state.status,
        true,
  ...
  if (state.sessionPicker) {
    if (state.sessionPicker.allProjects) {
      ...
      parts.push(renderPanel('projects', body))
    } else {
      ...
      parts.push(renderPanel('sessions', body))
    }
    parts.push(
      '',
      renderFooterBar(
...
async function handleInput(
  ...
  // Flush startup messages before first user message
  if (state.pendingStartupMessages.length > 0) {
    pushTranscriptEntry(state, {
      kind: 'assistant',
      body: state.pendingStartupMessages.join('\n'),
    })
...
export async function runTtyApp(args: TtyAppArgs): Promise<void> {
  ...
  // Collect startup info — don't push to transcript so logo screen shows first
  state.pendingStartupMessages = []
  const memoryFiles = await discoverInstructionFiles(args.cwd)
  if (memoryFiles.length > 0) {
    state.pendingStartupMessages.push(`Memory: ${memoryFiles.length} instruction file(s) loaded`)
    for (const f of memoryFiles) {
      const lineCount = f.content.split('\n').length
      const preview = f.content.trim().split('\n')[0] || '<empty>'
      state.pendingStartupMessages.push(`  ${f.path}  lines=${lineCount}  ${preview}`)
    }
  }
  ...
  if (permissionArgs.resumeTarget) {
    ...
    if (expired > 0) {
      state.pendingStartupMessages.push(`Cleaned up ${expired} expired session(s) (>30 days old).`)
    }
    const sessions = await listSessions(args.cwd)
    if (sessions.length > 0) {
      state.pendingStartupMessages.push(`Found ${sessions.length} saved session(s). Type /resume to continue one.`)
    }
  }
  ...
  await new Promise<void>(resolve => {
    ...
    const cleanup = () => {
      ...
      exitAlternateScreen()
      process.stdin.pause()
      process.stdout.write(`Session ${permissionArgs.sessionId} saved. To resume: PowerCode --resume ${permissionArgs.sessionId}\n`)
    }
    ...
    const handleEvent = async (event: ParsedInputEvent) => {
      try {
        ...
        if (event.kind === 'key' && event.name === 'return') {
          ...
          if (submitInFlight) {
            state.status = 'Previous command still finishing...'
            renderScreen(permissionArgs, state)
            return
          }
          ...
          state.selectedSlashIndex = 0
          // Push user message before render so working state appears immediately
          pushTranscriptEntry(state, { kind: 'user', body: submittedInput })
          submitInFlight = true
          renderScreen(permissionArgs, state)
          void (async () => {
            try {
