...
const USER_AGENT =
  'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 PowerCode/0.1'
...
/**
 * 搜索入口：并行请求多个 provider，取第一个有效结果
 * 比串行 fallback 快很多（搜狗 3-4 秒就返回，不用等 DuckDuckGo 超时）
 */
export async function searchDuckDuckGoLite(options: {
  ...
  // 并行请求所有 provider，取第一个有效结果
  const result = await Promise.any(
    providers.map(async (provider): Promise<{
      organic: SearchResult[]
      base_resp: { status_code: number; status_msg: string; source: string }
    }> => {
      ...
      if (organic.length === 0) {
        throw new Error(`${provider}: no results`)
      }
