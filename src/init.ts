...
const GITIGNORE_COMMENT = '# PowerCode local artifacts'
const GITIGNORE_ENTRIES = ['.powercode/settings.local.json', '.powercode/sessions/']
...
export function renderInitMiniMd(cwd: string): string {
  ...
  sections.push([
    '# MINI.md',
    '',
    'This file provides guidance to PowerCode when working with code in this repository.',
    '',
  ])
  ...
  sections.push([
    ...
    '- Keep shared defaults in `~/.powercode/settings.json`; reserve `.powercode/settings.local.json` for project-local overrides.',
...
export async function initializeRepo(cwd: string): Promise<InitReport> {
  ...
  const PowerCodeDir = path.join(cwd, '.powercode')
  artifacts.push({
    name: '.powercode/',
    status: await ensureDir(PowerCodeDir),
  })
