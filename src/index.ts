    ...
    for await (const rawInput of rl) {
      ...
      try {
        ...
        if (input === "/collapse") {
          ...
          contextCollapseState.consecutiveFailures =
          ...
          const savedTokens = result.spans.reduce(
            (sum, span) =>
              sum + Math.max(0, span.tokensBefore - span.tokensAfter),
            0,
          );
          console.log(
